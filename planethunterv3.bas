1 rem planet hunter v0.3 game
3 poke53280,11:poke53281,0:l=782:m=781:w=65520:s=1024:c=55296-s
5 forx=.to23:r$=r$+"{down}":next:gosub450
7 a$="IJKU ":rem old a$="IJKU {sh asterisk}{125}";arabic like?
8 printr$:gosub300:print:gosub400
...
24 print:print"{green}reading antenna data..";
25 rem forx=.to959:printmid$(a$,1+rnd(.)*5,1);:next:tt=0
26 forq=.to3:q$(q)="":forx=.to239:q$(q)=q$(q)+mid$(a$,1+rnd(1)*5,1):next:print".";:next
28 print"{home}{light gray}";:forq=.to3:printq$(q);:next:tt=0
...
35 pokel,0:pokem,24:sysw:print"{white}scanning... {light green}how many planets? (0-9): ";
...
85 ifp=0thenpokel,0:pokem,24:sysw:print"{pink}no planets found!";:goto91
90 pokel,0:pokem,24:sysw:print"{white}found"p"planet";:ifp>1thenprint"s";
91 print"{light green}, you input "z$;:ifz$=""thentt=rt
93 ifval(z$)=pthenprint"{yellow} correct!";:goto96
94 print"{red} wrong!":goto130:rem wrong answer, game over!
...
104 r=r+1:ifr=3orr=7orr=13orr=16orr=17thengosub118:rem rumors
105 goto10
...
127 if r=16then gosub145
128 if r=17then print"the planet scanner has failed...":goto130
129 return
...
138 ts=0:r=0:forx=.to3999:next:goto8:rem new game
